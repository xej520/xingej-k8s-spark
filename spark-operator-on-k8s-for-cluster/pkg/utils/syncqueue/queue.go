...
// GetQueueNum qqqq
func (sq *SyncQueue) GetQueueNum(obj interface{}) (num int) {
	return sq.queue.NumRequeues(obj)
}
...
func (sq *SyncQueue) handleSyncError(err error, obj interface{}) {
	...
	if sq.queue.NumRequeues(obj) < sq.maxRetries {
		log.Warnf("Error syncing object (type: %v, key: %v) retry: %v, err: %v",
	...
	log.Warnf("Dropping object (type: %v, key: %v) from the queue, err: %v", sq.syncType, key, err)
	sq.queue.Forget(obj)
}
